# /////////////////////////////////////////////////
# Active case finding, pre process
# /////////////////////////////////////////////////

# This R script is used run the simulation of the active case finding
...
test_dir_outputs <- file.path(test_dir, "intervention_acf")
...
for (i in seq_along(seq_hha)) {
  for(j in seq_along(seq_hhs)) {
    for(k in seq_along(seq_acf_screened)) {
  ...
  config_new$intervention$acf_screened <- seq_acf_screened[k]
  ...
  config_path <- file.path(
    test_dir_outputs,
    paste0(
      "hha_", seq_hha[i], "_hhs_", seq_hhs[j], "_acf_", seq_acf_screened[k]
    )
  )
